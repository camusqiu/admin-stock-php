...
class StockTagInitController extends CommonController {
    ...
    public function updateStockHYGNTag(){
        ...
        if($listTagInit == null){
            ...
            $dataCom['in_time'] = $this->is_list($code);
                ...
                if ($available && $available[0]['available'] == 1) {
                    ...
                    if ($a && $a != null) {
                        ...
                        if (substr($a[0]['out_time'],0,10)> substr($a[0]['in_time'],0,10)) {  
                            $shData['in_time'] = $this->is_list($code);
                        }

                    }else{
                        $shData['out_time'] = $now;

                        $ahData['in_time'] = $this->is_list($code);
                        $ahData['code'] = $code;
                        $ahData['ucode'] = $ucode;
                    ...
                    if ($index === false) {
                        ...
                        if ($available && $available[0]['available'] == 1) {
                            ...
                            if ($a && $a != null) {
                                ...
                                $aData['in_time'] = $this->is_list($code);
                                $aData['code'] = $code;
                                $aData['ucode'] = $ucode;
                                $aData['tag_id'] = $new[$j];
    ...
    public function is_list($code){
        $now = date("Y-m-d H:i:s");
        $Model = M('StockBar');
        $list = $Model->where("code='".$code."'")->select();
        if ($list && $list != null) {
            //未上市
            if($list[0]['state'] == 4){
                return "";
